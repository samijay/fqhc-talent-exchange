...
import { ConceptCard } from "../okr-course/exercises/ConceptCard";
import { ClassifierExercise } from "../okr-course/exercises/ClassifierExercise";
import { DragSortRanking } from "../okr-course/exercises/DragSortRanking";
import { MiniQuiz } from "../okr-course/exercises/MiniQuiz";
import type { MasterclassCourseModule, MasterclassExercise } from "@/lib/masterclass-course-modules";
import type { CourseProgress } from "@/lib/masterclass-course-progress";

interface MasterclassModuleScreenProps {
  module: MasterclassCourseModule;
...
export function MasterclassModuleScreen({
  module,
  onExerciseComplete,
  onModuleComplete,
  onBack,
}: MasterclassModuleScreenProps) {
  const locale = useLocale();
  const t = (obj: { en: string; es: string } | string) => {
  ...
  const handleExerciseComplete = useCallback(
    (exercise: MasterclassExercise, score: number) => {
  ...
  const renderExercise = (exercise: MasterclassExercise) => {
    ...
    <div className="flex flex-col gap-4 w-full max-w-2xl mx-auto">
      ...
      {phase === "complete" && (
        <div className="flex flex-col items-center text-center gap-6 py-8">
          ...
          {module.sourceMaterials.length > 0 && (
            <Card className="w-full max-w-md text-left border-stone-200 dark:border-stone-700">
              <CardContent className="p-4">
                <p className="text-sm font-medium text-stone-600 dark:text-stone-500 mb-3">
                  {locale === "es"
                ...
                <div className="space-y-2">
                  {module.sourceMaterials.map((material, i) => (
                    <a
                      key={i}
                      href={material.url}
                      target="_blank"
