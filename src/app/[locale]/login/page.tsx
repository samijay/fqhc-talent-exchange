...
function LoginForm() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
          ...
          t(
            {
              ...
              es: "El inicio de sesión con Google falló. Intenta de nuevo.",
            },
            locale
          )
        );
      ...
      // If no error, the browser will redirect to Google — don't reset loading
    } catch (err) {
      console.error("[login] Unexpected Google OAuth error:", err);
      toast.error(
        t(
          {
            en: "Something went wrong. Please try again.",
  ...
  return (
    <div className="w-full max-w-md space-y-8">
      ...
      {authError && (
        <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">
          {authError === "no_code"
            ? t(
                {
                  en: "Authentication was cancelled or failed. Please try again.",
                  es: "La autenticación fue cancelada o falló. Intenta de nuevo.",
                },
                locale
              )
            : authReason
              ? t(
                  {
                    en: `Authentication failed: ${decodeURIComponent(authReason)}. Please try again.`,
                    es: `La autenticación falló: ${decodeURIComponent(authReason)}. Intenta de nuevo.`,
                  },
                  locale
                )
              : t(
      ...
      >
        ...
        {googleLoading
          ? t({ en: "Redirecting to Google...", es: "Redirigiendo a Google..." }, locale)
          : t({ en: "Continue with Google", es: "Continuar con Google" }, locale)}
      </button>
