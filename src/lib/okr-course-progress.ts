// okr-course-progress.ts
// localStorage-based progress tracking for the OKR individual course
...
export interface CourseProgress {
  ...
  capstoneSubmitted: OKRDraft | null;
  startedAt: string;
  lastActiveAt: string;
}
...
const STORAGE_PREFIX = "okr-course-progress";
...
/** Check if all prerequisite modules are done (for capstone gate) */
export function canAccessCapstone(
  progress: CourseProgress,
  prerequisiteModuleIds: string[]
): boolean {
  return prerequisiteModuleIds.every((id) =>
    progress.modulesCompleted.includes(id)
  );
}
