...
| 91 | **Salary Negotiation Masterclass** | Medium | Content/B2B | Beef up existing `/blog/fqhc-salary-negotiation-guide` into a comprehensive masterclass-level resource balanced for BOTH sides: executives/managers AND labor/candidates. Cover: (1) Where common ground exists (retention ROI, total comp vs base salary, mission alignment), (2) Where tension is real and how to work through it (PPS constraints, grant-funded caps, union vs non-union dynamics, SB 525 compliance), (3) FQHC-specific negotiation levers (NHSC loan repayment, bilingual premiums, ECM/CalAIM program revenue, schedule flexibility, professional development budgets). Make it the definitive guide that both HR directors and CHWs bookmark. Consider adding to masterclass library at `/strategy/masterclass`. |
...
| 93 | **SEO Quick Wins** | Medium | SEO | Enable hreflang tags (uncomment ES locale in seo-config), create llms.txt for AI discoverability, add FAQ schema to guides/resources, add Article author to blog JSON-LD, expand breadcrumb schema to all pages. |
...
| 99 | **Weekly Roundup Command (`/weekly-roundup`)** | High | Content/Newsletter | New slash command that runs weekly (Fridays?) to prepare newsletter content: (1) **Look back**: summarize the week's intel items, policy changes, layoff updates, new jobs added, (2) **Look forward**: scan for upcoming deadlines, funding cliffs, policy effective dates, (3) **Site analytics**: check GA4 for most-visited pages, popular tools, trending content (requires GA4 API or CSV export), (4) **Featured jobs**: identify high-demand or new postings from the week, (5) **Cutting edge**: scan for latest FQHC innovation, AI adoption, best practices, (6) **Output**: structured content ready to drop into Intel Brief + The Pulse newsletter templates. Should generate both employer-facing and candidate-facing versions. Feeds into `/intel-brief` command. |
